            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6 w-full">
              {filteredItems.map((menu) => {
                ...
                return (
                  <Card
                    key={menu.id}
                    className={`group overflow-hidden rounded-xl border bg-card text-card-foreground shadow-sm transition-all duration-300 flex flex-col p-0 relative
                    ${
                      isAvailable
                    ...
                    <div className="relative w-full aspect-16/10 overflow-hidden bg-muted">
                      ...
                      {!isAvailable && (
                        <div className="absolute inset-0 bg-background/40 backdrop-blur-[1px] flex items-center justify-center">
                          <Badge
                            variant="destructive"
                            className="px-3 py-1 text-sm font-bold uppercase tracking-wider border-none bg-destructive/90 text-white shadow"
                          >
                            Unavailable
                          </Badge>
                        </div>
